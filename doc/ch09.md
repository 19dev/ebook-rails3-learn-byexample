...
signin gerekecek biçimde index sayfasını tasarlayalım,
...
controller'da filtreye bu durumu bildir,
...
kullanıcılar listelenirken `<li>` içerisinde olacak,
...
controllers'da index'in tüm kullanıcıları gösterecek biçiminde sorgu sonucunu tuttur,
...
view'i `@users`u işleyecek hale getirelim,
...
~$ vim app/assests/stylesheets/custom.css.scss -> layout.sass
.users
  ...

navigasyon panelinde Users'a link olmalı
...
test kullanıcılarını ekleyecek kod,

~$ vim lib/tasks/sample_data.rake
namespace :db do
...
view'i buna göre tasarlayalım

~$ vim app/views/users/index.html.erb
<%= will_paginate %>

controller'da da index'i güncelle
~$ vim app/controllers/users_controller.rb
def index
...
~$ vim app/views/users/index.html.erb
...
~$ vim app/views/users/user.html.erb
...
~$ vim app/views/users/index.html.erb
...
~$ bundle exec rspec spec/
...
~$ vim db/migrate/20120301004903_add_admin_to_users.rb
...
